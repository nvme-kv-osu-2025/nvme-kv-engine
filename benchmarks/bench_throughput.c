...
void print_results(double start_time, int write_success, int write_fail) {
  ...
  double write_latency_us =
      (write_time * 1000000.0) / (write_success + write_fail);
  double write_bandwidth_mbps =
      ((write_success + write_fail) * VALUE_SIZE) / (write_time * 1024 * 1024);
  double write_success_rate =
      (double)write_success / (double)(write_success + write_fail);
  ...
  printf("  Success: %d, Failures: %d, Success rate: %.2f%%\n", write_success,
         write_fail, write_success_rate * 100.0);

  /* CSV-friendly line for automation */
  // printf("CSV,%d,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f\n",
  //     num_ops,
  //     write_throughput, write_latency_us, write_bandwidth_mbps,
  //     write_success_rate * 100.0, read_throughput, read_latency_us,
  //     read_bandwidth_mbps, read_success_rate * 100.0);
}
...
int throughput_testing(kv_engine_t *engine, int num_ops) {
  ...
  for (int i = 0; i < num_ops; i++) {
    ...
    kv_result_t result =
        kv_engine_store(engine, key_buffer, KEY_SIZE, value_buffer, VALUE_SIZE,
                        true); /* benchmark needs to overwrite */
  ...
  for (int i = 0; i < num_ops; i++) {
    ...
    kv_result_t result = kv_engine_retrieve(engine, key_buffer, KEY_SIZE,
                                            &retrieved, &retrieved_len, false);
    if (result == KV_SUCCESS) {
      read_success++;
      kv_engine_free_buffer(engine, retrieved);
    } else {
      read_fail++;
    }
