...
#define DEFAULT_DEVICE "/dev/kvemul"
...
static void *stats_updater_thread(void *arg) {
    ...
    for (int i = 0; i < args->num_updates; i++) {
        ...
        update_stats(args->engine, 1, 0, 0, 1, 100);  /* Read success */
        update_stats(args->engine, 0, 1, 0, 1, 200);  /* Write success */
        update_stats(args->engine, 0, 0, 1, 1, 0);    /* Delete success */
        
        if (i % 10 == 0) {
            update_stats(args->engine, 1, 0, 0, 0, 0); /* Read failure */
        }
    }
