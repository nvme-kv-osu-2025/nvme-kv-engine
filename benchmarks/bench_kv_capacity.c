...
static int parse_args(int argc, char **argv, cfg_t *c) {
  ...
  for (uint32_t i = 0; i < c->key_sizes_count; i++) {
    if (c->key_sizes[i] < 4) c->key_sizes[i] = 4;
    if (c->key_sizes[i] > 255) c->key_sizes[i] = 255;
  }
  for (uint32_t i = 0; i < c->value_sizes_count; i++) {
    if (c->value_sizes[i] < 1) c->value_sizes[i] = 1;
    if (c->value_sizes[i] > 2 * 1024 * 1024) c->value_sizes[i] = 2 * 1024 * 1024;
  }
  if (c->max_keys == 0) c->max_keys = 1;
  if (c->max_seconds <= 0.0) c->max_seconds = 1.0;
  if (c->failure_threshold == 0) c->failure_threshold = 1;
  return 0;
}
...
static void make_key(char *buf, uint32_t key_len, uint64_t idx,
                     uint32_t k_size, uint32_t v_size, uint32_t seed) {
  /* Use fixed prefix derived from (k_size, v_size, seed) so collisions across
   * combos are impossible. */
  char prefix[32];
  int n = snprintf(prefix, sizeof(prefix), "k%03u_v%07u_s%08x_",
                   k_size, v_size, seed);
  if (n < 0) n = 0;
  size_t plen = (size_t)n;
  if (plen > key_len) plen = key_len;
  ...
  if (plen + hlen > key_len) hlen = key_len - plen;
...
static int run_combo(const cfg_t *cfg, const bench_device_list_t *devices,
  ...
  if (bench_init_engine_multi(&engine, devices, &opts) != KV_SUCCESS) {
    fprintf(stderr,
            "[capacity] init failed for k=%u v=%u\n", key_size, value_size);
  ...
  fill_value(vbuf, value_size, ((uint64_t)cfg->seed << 16) ^
                                ((uint64_t)key_size << 24) ^ value_size);
  ...
  for (; i < cfg->max_keys; i++) {
    ...
    if (res == KV_SUCCESS) {
      ...
      out->last_error = res;
      if (out->first_failure_at == 0) out->first_failure_at = i + 1;
  ...
  if (i >= cfg->max_keys) out->reached_cap = true;

  getrusage(RUSAGE_SELF, &ru_after);
  out->user_cpu_sec = timeval_to_sec(ru_after.ru_utime) -
                       timeval_to_sec(ru_before.ru_utime);
  out->sys_cpu_sec = timeval_to_sec(ru_after.ru_stime) -
                      timeval_to_sec(ru_before.ru_stime);
...
static void emit_csv(const cfg_t *cfg, const combo_result_t *r) {
  ...
  fprintf(fp,
          ",%u,%u,%" PRIu64 ",%.3f,%" PRIu64 ",%" PRIu64 ",%s,%.4f,%.2f,%" PRIu64
          ",%.3f,%.3f,%ld,",
...
static void emit_summary(const cfg_t *cfg, const combo_result_t *r) {
  ...
  fprintf(fp,
          ...
          r->ops_per_sec, r->total_bytes,
          bench_kv_result_string(r->last_error),
          r->stopped_on_failure
              ? " (stopped on failure)"
              : (r->reached_cap ? " (cap reached)"
