...
static int parse_workload(const char *s, workload_t *out) {
  if (!s || !out) return -1;
  if (!strcmp(s, "write")) *out = WL_WRITE;
  else if (!strcmp(s, "read")) *out = WL_READ;
  else if (!strcmp(s, "mixed")) *out = WL_MIXED;
  else if (!strcmp(s, "delete")) *out = WL_DELETE;
  else if (!strcmp(s, "exists")) *out = WL_EXISTS;
  else if (!strcmp(s, "edge")) *out = WL_EDGE;
  else return -1;
  return 0;
}
...
static int parse_args(int argc, char **argv, cfg_t *c) {
  ...
  if (c->key_min < 4) c->key_min = 4;
  if (c->key_max > 255) c->key_max = 255;
  if (c->key_max < c->key_min) c->key_max = c->key_min;
  if (c->value_min < 1) c->value_min = 1;
  if (c->value_max > (2 * 1024 * 1024)) c->value_max = 2 * 1024 * 1024;
  if (c->value_max < c->value_min) c->value_max = c->value_min;
  if (c->threads < 1) c->threads = 1;
  if (c->read_percent < 0) c->read_percent = 0;
  if (c->read_percent > 100) c->read_percent = 100;
  if (c->keyspace == 0) c->keyspace = 1;
  return 0;
}
...
static void gen_key(char *buf, size_t key_len, uint64_t idx) {
  ...
  if (hex_len > key_len) hex_len = key_len;
...
static int do_read(kv_engine_t *engine, char *kbuf, rng_t *rng,
  ...
  if (res == KV_SUCCESS) {
    if (out_value_len) *out_value_len = value_len;
...
static void *worker_entry(void *arg) {
  ...
  for (;;) {
    if (budget && local_done >= budget) break;
    if (now_seconds() >= end) break;
...
static int prepopulate(kv_engine_t *engine, const cfg_t *cfg) {
  ...
  for (uint64_t i = 0; i < cfg->keyspace; i++) {
    ...
    if (res != KV_SUCCESS) {
      ...
      if (fail < 5) {
        fprintf(stderr,
                "[stress] prepopulate failed at %" PRIu64 " (%s)\n", i,
                bench_kv_result_string(res));
      }
    }
  }
  double dt = now_seconds() - t0;
  fprintf(stderr, "[stress] prepopulate complete in %.2fs (%" PRIu64
                   " failures)\n",
          dt, fail);
  free(kbuf);
  free(vbuf);
...
static void capture_rusage(struct rusage *before, struct rusage *after,
                           run_result_t *result) {
  result->user_cpu_sec = timeval_to_sec(after->ru_utime) -
                          timeval_to_sec(before->ru_utime);
  result->sys_cpu_sec = timeval_to_sec(after->ru_stime) -
                         timeval_to_sec(before->ru_stime);
...
static void emit_csv(const cfg_t *cfg, const run_result_t *r) {
  ...
  fprintf(fp,
          ...
          cfg->key_max, cfg->value_min, cfg->value_max, cfg->keyspace, cfg->seed,
...
static void emit_summary(const cfg_t *cfg, const run_result_t *r) {
  ...
  fprintf(fp,
          ...
          avg_us,
          (double)bench_latvec_percentile(&r->latencies, 0.50) / 1000.0,
...
static int run_edge_cases(kv_engine_t *engine, const cfg_t *cfg) {
  ...
  /* 2. Max-length key (255 bytes), small value */
  {
    ...
    char key[255];
    for (int i = 0; i < 255; i++) key[i] = 'a' + (i % 26);
    uint64_t t0 = bench_time_ns();
    kv_result_t res =
        kv_engine_store(engine, key, 255, vbuf_small, 64, true);
    if (res == KV_SUCCESS) {
      void *out = NULL;
      size_t out_len = 0;
  ...
  /* 3. Small value (1 byte) */
  {
    ...
    if (res == KV_SUCCESS) {
      ...
      if (res == KV_SUCCESS) {
        er.passed = (out_len == 1);
        if (!er.passed) er.detail = "unexpected length";
        kv_engine_free_buffer(engine, out);
      } else {
        er.detail = bench_kv_result_string(res);
      }
  ...
  {
    edge_result_t er = {"no_overwrite_collision",
                        "expects KEY_ALREADY_EXISTS", 0, 0, ""};
    ...
    if (fp) {
      for (size_t i = 0; i < nr; i++) {
        ...
        fprintf(fp,
                ...
                lat_us, lat_us, lat_us, lat_us, lat_us, lat_us,
                cfg->key_min, cfg->key_max, cfg->value_min, cfg->value_max,
                cfg->seed);
        char note[160];
...
int main(int argc, char **argv) {
  ...
  if (cfg.warmup_sec > 0.0) {
    ...
    if (kbuf && vbuf) {
      ...
      while (now_seconds() < end) {
        ...
        switch (op) {
        case WL_WRITE:
          do_write(engine, kbuf, vbuf, &rng, &cfg, key_idx, &bytes); break;
        case WL_READ:
          do_read(engine, kbuf, &rng, &cfg, key_idx, &bytes); break;
        case WL_DELETE:
          do_delete(engine, kbuf, &rng, &cfg, key_idx); break;
        case WL_EXISTS:
          do_exists(engine, kbuf, &rng, &cfg, key_idx); break;
        default:
          break;
        }
        i++;
  ...
  double t_end = (cfg.ops > 0) ? (t_start + 24.0 * 3600.0)
                               : (t_start + cfg.duration_sec);
  ...
  for (uint32_t i = 0; i < cfg.threads; i++) {
    ...
    if (cfg.workload == WL_DELETE) {
      ...
      workers[i].key_count = (i * slice + slice <= cfg.keyspace)
                                  ? slice
                                  : (cfg.keyspace > i * slice
                                         ? cfg.keyspace - i * slice
                                         : 0);
      if (workers[i].key_count == 0) workers[i].key_count = 1;
    } else {
