...
#define NUM_ASYNC_OPS 10
  ...
  for (int i = 0; i < NUM_ASYNC_OPS; i++) {
    ...
    kv_result_t result =
        kv_engine_store_async(engine, key, strlen(key), value, strlen(value),
                              store_callback, &contexts[i], false);
  ...
  for (int i = 0; i < NUM_ASYNC_OPS; i++) {
    ...
    kv_result_t result = kv_engine_delete_async(engine, key, strlen(key),
                                                delete_callback, &contexts[i]);
